              ...
              <Table.ActionPanel>
                ...
                <Justify
                  left={
                    <Form layout='inline'>
                      {duck.monitorLabels
                        .filter(labelKey => LabelKeyMap[labelKey].role.indexOf('caller') !== -1)
                        .map(labelKey => {
                          ...
                          return (
                            <FormItem key={labelKey} label={LabelKeyMap[labelKey].text.replace('主调', '')}>
                              {
                                <InputAdornment
                                  after={
                ...
                />
                {duck.monitorLabels.filter(labelKey => LabelKeyMap[labelKey].role.indexOf('callee') !== -1).length >
                  0 &&
                  duck.type !== 'ratelimit' && (
                    <Text theme={'label'} parent={'div'} style={{ marginBottom: '10px' }}>
                      被调方
                    ...
                    <Form layout='inline'>
                      {duck.monitorLabels
                        ...
                        .map(labelKey => {
                          ...
                          const { data: options, loading } = fetcher.selector(store)
                          return (
                            <FormItem key={labelKey} label={LabelKeyMap[labelKey].text.replace('被调', '')}>
                              {
                                <InputAdornment
                                  after={
                                    filterConfig.filterLabels[labelKey]?.length > 0 ? (
